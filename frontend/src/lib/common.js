...
function formatBooks(bookArray) {
  return bookArray.map((book) => {
    const newBook = { ...book };
    // use MongoDB _id if present, otherwise fall back to id field
    // eslint-disable-next-line no-underscore-dangle
    newBook.id = newBook._id || newBook.id;
    // if image is served locally, prefix with API URL so browser hits backend
    if (newBook.imageUrl && newBook.imageUrl.startsWith('/images')) {
      newBook.imageUrl = `${API_ROUTES.BOOKS.replace('/api/books', '')}${newBook.imageUrl}`;
    }
    return newBook;
  });
}
...
export async function getBook(id) {
  try {
    ...
    // prefix image URL if local
    if (book.imageUrl && book.imageUrl.startsWith('/images')) {
      book.imageUrl = `${API_ROUTES.BOOKS.replace('/api/books', '')}${book.imageUrl}`;
    }
    return book;
  } catch (err) {
    console.error(err);
